...
static void process_cursor_motion(Compositor* comp, uint32_t time) {
    ...
    // A button is already held (implicit grab): keep sending motion to
    // whatever surface got the initial press, even if the cursor briefly
    // strays over something else mid-click. Re-resolving here would send a
    // spurious "enter" to a different surface and read to the app as
    // "clicked outside me" — e.g. a Steam dialog dismissing itself on a
    // tiny mouse jitter during the click.
    if (comp->seat->pointer_state.button_count > 0) {
        double sx = comp->cursor->x - comp->grab_surface_ox;
        double sy = comp->cursor->y - comp->grab_surface_oy;
...
static void cursor_button(struct wl_listener* listener, void* data) {
    ...
    if (surface) {
        comp->grab_surface_ox = comp->cursor->x - sx;
        comp->grab_surface_oy = comp->cursor->y - sy;
    }
    ...
    wlr_seat_pointer_notify_button(comp->seat, event->time_msec, event->button, event->state);
    // Override-redirect X11 windows (Steam popups, tooltips, context menus)
    // manage their own X-level input focus and aren't part of normal window
    // management — forcing our activate/deactivate cycle onto them was
    // knocking Steam's real window out of "activated" state on every click,
    // which Steam apparently reads as "clicked away" and self-dismisses.
    bool is_override_redirect = (kind == WINDOW_KIND_XWAYLAND) && win &&
        ((XWindow*)win)->xsurface->override_redirect;
